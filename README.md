# pawan-portfolio
# 👋 Hi, I'm Pawan Jogi

🎓 B.Tech Student  
📊 Aspiring Data Analyst  
💻 Passionate about Data, Technology and Problem Solving

---

# 🚀 About Me
I am currently learning Data Analytics and building projects using real-world datasets.  
I enjoy turning raw data into meaningful insights.

---

# 🛠 Skills

**Programming**
- Python
- SQL

**Data Analysis**
- Excel
- Pandas
- NumPy

**Data Visualization**
- Power BI
- Matplotlib
- Seaborn

---

# 📂 Projects

## 📊 Sales Data Analysis
**Tools:** Python, Pandas, Matplotlib  
- Analyzed sales dataset  
- Found top products and trends  
- Created visualizations  

🔗 Project Link: (Add GitHub link)

---

## 🎬 Netflix Data Analysis
**Tools:** Python  
- Explored Netflix dataset  
- Genre analysis  
- Content trends visualization  

🔗 Project Link: (Add link)

---

## 🛒 Customer Segmentation
**Tools:** Python, Machine Learning  
- Used clustering to group customers  
- Identified purchasing patterns  

🔗 Project Link: (Add link)

---

# 📜 Certifications
- Android App Development – Udemy
- (Add more here)

---

# 📈 Currently Learning
- Data Science
- Machine Learning
- Advanced SQL

---

# 📫 Contact Me
📧 Email: pjpawan007@gmail.com
💼 LinkedIn: [inkedin.com/in/pawan-jogi-716992338/](https://www.linkedin.com/in/pawan-jogi-716992338/)
💻 GitHub: your profile link : https://github.com/PawanJogi07/pawan-portfolio/edit/main/README.md
